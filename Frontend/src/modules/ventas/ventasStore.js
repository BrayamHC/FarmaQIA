...
export const useVentasStore = defineStore('ventas', () => {
  ...
  const subtotal = computed(() =>
    carrito.value.reduce(
      (acc, item) =>
        acc + Number(item.cantidad ?? 0) * Number(item.precio_unitario ?? item.preciounitario ?? 0),
      0,
    ),
  )
  ...
  function normalizarLotes(producto) {
    ...
    return lotes
      .map((lote) => ({
        ...
        almacenes: Array.isArray(lote?.almacenes) ? lote.almacenes : [],
      }))
  ...
  function obtenerPrecioUnitario(producto) {
    ...
    if (Array.isArray(producto.precios) && producto.precios.length) {
      const precioDefault =
        ...
        producto.precios.find((item) => item?.esdefault) ??
      ...
      if (precioDefault?.precioventa != null) {
        return Number(precioDefault.precioventa ?? 0)
      }
    }

    return Number(producto.precio_publico ?? producto.preciopublico ?? 0)
  }

  function obtenerStockLista(producto) {
    if (!producto) return null

    if (producto.stock_disponible != null) return Number(producto.stock_disponible ?? 0)
    if (producto.stockdisponible != null) return Number(producto.stockdisponible ?? 0)

    if (producto?.stock?.total != null) return Number(producto.stock.total ?? 0)
  ...
  function obtenerStockDisponibleProducto(producto) {
    const stock = obtenerStockLista(producto)
  ...
  async function buscarProductoParaPOS(termino = '') {
    ...
    if (esTextoCodigo(texto)) {
      ...
      const productosUpc = porUpc?.productos ?? []
      if (productosUpc.length) return productosUpc
    }
  ...
  async function obtenerProductoParaVenta(productoUuid) {
    ...
    try {
      ...
      productoSeleccionado.value = {
        ...response,
        con_lote: Boolean(response?.con_lote ?? response?.conlote),
        conlote: Boolean(response?.conlote ?? response?.con_lote),
        lotes: normalizarLotes(response),
        precio_unitario: obtenerPrecioUnitario(response),
        preciounitario: obtenerPrecioUnitario(response),
  ...
  async function seleccionarProducto(productoBase) {
    const productoUuid = productoBase?.producto_uuid ?? productoBase?.productouuid
    if (!productoUuid) return null
    return await obtenerProductoParaVenta(productoUuid)
  }
  ...
  function generarKeyCarrito(producto, lote = null) {
    const productoUuid = producto?.producto_uuid ?? producto?.productouuid
    return lote?.lote_uuid
      ? `${productoUuid}_${lote.lote_uuid}`
      : `${productoUuid}_sin_lote`
  }

  function agregarProductoAlCarrito({ producto, lote = null, cantidad = 1 }) {
    if (!producto?.producto_uuid && !producto?.productouuid) return
    ...
    carrito.value.push({
      _key: key,
      key,
      producto_uuid: producto.producto_uuid ?? producto.productouuid,
      productouuid: producto.productouuid ?? producto.producto_uuid,
      producto,
  ...
  function actualizarCantidadProducto(key, cantidad) {
    const item = carrito.value.find((row) => row._key === key || row.key === key)
    if (!item) return
  ...
  function eliminarProductoDelCarrito(key) {
    carrito.value = carrito.value.filter((item) => item._key !== key && item.key !== key)
  }
