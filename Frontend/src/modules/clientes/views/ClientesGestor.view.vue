    ...
    <header class="flex flex-col gap-3">
      ...
      <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-sm text-slate-500">
              Administre clientes, datos fiscales y medios de contacto.
            </p>
          </div>
        </div>
        ...
        <button
          ...
          @click="router.push({ name: 'CrearCliente' })">
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">
        <div class="xl:col-span-2">
          ...
          <div class="relative">
            ...
            <input v-model="filtros.q" type="text" placeholder="Nombre o RFC..." class="farma-search-input"
              @input="onBuscarInput" />
          </div>
        </div>
    ...
    <article class="card-base farma-table-shell flex min-h-0 flex-1 flex-col">
      <div class="farma-table-content app-scroll flex-1 min-h-0">
        <DataTable :value="clientesTabla" scrollable scrollHeight="flex" dataKey="cliente_uuid"
          ...
          <template #empty>
            <div class="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p class="text-sm font-medium text-slate-500">
                {{ clientesStore.cargando ? 'Cargando clientes...' : 'No se encontraron clientes' }}
              </p>
              <p class="mt-1 text-xs text-slate-400">
                {{ clientesStore.cargando ? 'Espera un momento' : 'Intenta ajustar los filtros de búsqueda' }}
              </p>
            </div>
          </template>

          <Column field="nombre" header="Cliente" style="width: 240px">
            <template #body="slotProps">
              <div>
                <p class="text-sm font-semibold text-slate-800">{{ slotProps.data.nombre || '—' }}</p>
                <p class="text-xs text-slate-400">
                  {{ slotProps.data.razon_social || 'Sin razón social' }}
                </p>
              </div>
            </template>
          </Column>
          ...
          <Column field="email" header="Correo" style="width: 240px">
            <template #body="slotProps">
              <span class="text-sm text-slate-600">{{ slotProps.data.email || '—' }}</span>
            </template>
          </Column>

          <Column field="codigo_postal_fiscal" header="C.P. Fiscal" style="width: 120px">
            <template #body="slotProps">
              <span class="text-sm text-slate-600">
                {{ slotProps.data.codigo_postal_fiscal || '—' }}
              </span>
            </template>
          </Column>
            ...
            <template #body="slotProps">
              <div class="flex items-center gap-2">
                <RouterLink :to="`/clientes/${slotProps.data.cliente_uuid}/editar`"
                  class="farma-action-btn text-blue-600 hover:bg-blue-50" title="Editar cliente">
...
const totalRegistros = computed(() => Number(clientesStore.total || 0));
const paginaActual = computed(() => Math.floor(first.value / rows.value) + 1);
const clientesTabla = computed(() =>
...
async function confirmarCambioStatus(cliente, status) {
  ...
  try {
    await clientesStore.cambiarStatusCliente(cliente.cliente_uuid, status);
    await cargarClientes();
  } catch (_error) { }
}
...
.clientes-table :deep(.p-datatable) {
  height: 100%;
}

.clientes-table :deep(.p-datatable-table-container) {
  height: 100%;
}

.clientes-table :deep(.p-datatable-wrapper) {
  height: 100%;
  overflow: auto;
...
.clientes-table :deep(.p-datatable-wrapper::-webkit-scrollbar) {
  width: 6px;
  height: 6px;
}

.clientes-table :deep(.p-datatable-wrapper::-webkit-scrollbar-track) {
  background: transparent;
  border-radius: 9999px;
}

.clientes-table :deep(.p-datatable-wrapper::-webkit-scrollbar-thumb) {
...
.clientes-table :deep(.p-datatable-wrapper::-webkit-scrollbar-thumb:hover) {
  background: var(--color-primary);
  border: 2px solid transparent;
  background-clip: padding-box;
}

.clientes-table :deep(.p-datatable-thead > tr > th) {
...
.clientes-table :deep(.p-datatable-tbody > tr:last-child > td) {
  border-bottom: none;
}

.clientes-table :deep(.p-datatable-tbody > tr:hover > td) {
