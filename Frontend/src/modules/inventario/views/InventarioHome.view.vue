  ...
  <section class="space-y-10">
    ...
    <header class="flex flex-col gap-3">
      <nav class="flex items-center gap-1.5 text-xs text-slate-400">
        ...
        <span class="font-medium text-emerald-600">Inventario</span>
      </nav>
    ...
    <section>
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <RouterLink v-for="sub in inventarioStore.submodulos" :key="sub.titulo" :to="sub.ruta"
          class="group relative flex flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500">
          <div
            class="absolute right-4 top-4 flex h-7 w-7 items-center justify-center rounded-full bg-slate-100 transition group-hover:bg-emerald-100">
            <i class="pi pi-arrow-right text-[11px] text-slate-400 transition group-hover:text-emerald-600"></i>
          </div>

          <div class="mb-4 flex h-11 w-11 items-center justify-center rounded-xl bg-emerald-50">
            <i :class="[sub.icono, 'text-lg text-emerald-600']"></i>
          </div>
          ...
          <div class="mt-4 flex flex-wrap gap-1.5">
            <span v-for="tag in sub.tags" :key="tag"
              class="rounded-full bg-emerald-50 px-2.5 py-0.5 text-[11px] font-medium text-emerald-600">
              {{ tag }}
            </span>
          </div>
        </RouterLink>
      ...
      <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm xl:col-span-2">
        <div class="mb-5 flex items-center justify-between">
          ...
          <RouterLink to="/productos/gestor"
            class="text-xs font-medium text-emerald-600 transition hover:text-emerald-700">
            Ver catálogo →
          </RouterLink>
        </div>
              ...
              <tbody class="divide-y divide-slate-50 bg-white">
                <tr v-for="item in inventarioStore.productosRecientes" :key="item.sku"
                  ...
                  <td class="py-3 pl-4 pr-4">
                    <span class="font-mono text-[13px] font-medium text-emerald-600">{{ item.sku }}</span>
                  </td>
      ...
      <div class="flex flex-col gap-4">
        <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div class="flex flex-col gap-2">
            <RouterLink to="/productos/gestor"
              class="flex items-center gap-3 rounded-xl border border-slate-200 px-4 py-3 text-sm font-medium text-slate-700 transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700">
        ...
        <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div class="flex items-start justify-between gap-3">
            <div>
              <p class="text-[10px] font-semibold uppercase tracking-widest text-emerald-600">Estado del inventario</p>
            ...
            <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-emerald-50">
              <i class="pi pi-warehouse text-base text-emerald-600"></i>
            </div>
          </div>
